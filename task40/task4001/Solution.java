...
public class Solution {
    ...
    public void sendPost(URL url, String urlParameters) throws Exception {
        ...
        connection.setDoOutput(true);//установка doOutput = true требуется для POST- и PUT- запросов

        OutputStream outputStream = connection.getOutputStream();//получаем outputStream
        outputStream.write(urlParameters.getBytes());//запихиваем параметры в outputStream
        //outputStream.flush(); - неплохо бы "спустить" то что записали но валидатор принял и без этого
