...
public class Solution {
    ...
    public static void setAttachment(Message message, String filename) throws MessagingException {
        MimeMultipart mmp = new MimeMultipart();              //создаем Multipart объект
        MimeBodyPart mbp = new MimeBodyPart();                //такие объекты собираются из частей MimeBodyPart, но у нас по заданию такая часть одна
        FileDataSource fds = new FileDataSource(filename);    //по названию понятно что этот объект является источником данных
        mbp.setDataHandler(new DataHandler(fds));             //устанавливаем через матрешку наш файл в MimeBodyPart
        mbp.setFileName(fds.getName());                       //устанавливаем туда же имя файла
        mmp.addBodyPart(mbp);                                 //добавляем часть(или части) в объект MimeMultipart
        message.setContent(mmp);                              //настраиваем MimeMultipart объект в качестве содержимого нашего сообщения
    }   //вот тут все отлично описано - https://ru.wikipedia.org/wiki/JavaMail
}
