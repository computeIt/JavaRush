...
//на самом деле вывод не соответствует заданию (XXX.class) но валидатор принимает
...
Блокирующая очередь, в которой каждая операция добавления должна ждать соответствующей операции удаления в другом потоке и наоборот. 
Синхронная очередь не имеет никакой внутренней емкости, даже емкости в один элемент.
В сущности, SynchronousQueue – это еще одна реализация упомянутого выше интерфейса BlockingQueue. 
Она предоставляет необычайно легковесный способ обмена одиночными элементами между потоками посредством семантики блокировки, 
используемой в ArrayBlockingQueue.
SynchronousQueue позволяет вставить элемент в очередь только в том случае, если имеется поток, 
ожидающий поступления элемента для обработки*/
