...
public class Solution {
    ...
    public static <T> T convertFromXmlToNormal(String xmlData, Class<T> clazz) throws IOException, JAXBException {
        JAXBContext jaxbContext = JAXBContext.newInstance(clazz);           //по ожидаемому классу создаем контекст
        Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();       //из него делаем unmarshaller

        StringReader reader = new StringReader(xmlData);                    //xml-строку засовываем с stringreader

        return (T) unmarshaller.unmarshal(reader);      //и кастируем полученное чудо до нашегоискомго и ожидаемого на выходе типа clazz
    }
